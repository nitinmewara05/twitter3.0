...
contract StatusUpdate {
   ...
   event StatusUpdate(address indexed user, string newStatus, uint256 timestamp);
   function setStatus(string memory _status) public{
    require(bytes(_ststus).length <= MAX_CHARACTER_AMOUNT, "Status is too long");
    statuses(msg.sender) = _ststus;
    emit StatusUpdate(msg.sender, _status, block.timestamp);
   }
